#!/usr/bin/env bash
# Host-only: converge Host Volume at /var/lib/prefect (ADR-0031).
...
DEVICE="${1:-}"
TARGET="${PREFECT_HOST_VOLUME_TARGET:-/var/lib/prefect}"
# Per-attempt device wait; longer horizon is Restart=on-failure + start-limit.
WAIT_SECONDS="${PREFECT_HOST_VOLUME_DEVICE_WAIT_SECONDS:-20}"
...
self_enable() {
  ...
  if [[ "${TARGET}" != "/var/lib/prefect" ]]; then
    return 0
  fi
  mkdir -p /etc/systemd/system/multi-user.target.wants
  ln -sfn /etc/systemd/system/prefect-host-volume.service \
    /etc/systemd/system/multi-user.target.wants/prefect-host-volume.service
}
