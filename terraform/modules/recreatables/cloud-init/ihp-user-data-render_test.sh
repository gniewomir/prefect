...
RENDER_MODULE="${REPO_ROOT}/terraform/modules/recreatables/cloud-init/render"
...
port = by_path['/etc/ssh/sshd_config.d/99-prefect-port.conf']
abort('missing Port drop-in') unless port
abort('Port drop-in wrong value') unless port['content'].to_s.include?(\"Port #{ssh_port}\")

script = by_path['/usr/local/lib/prefect/ensure-host-volume-mount.sh']
...
unit = by_path['/etc/systemd/system/prefect-host-volume.service']
abort('missing prefect-host-volume.service') unless unit
...
abort('missing tmpfiles WantedBy recipe') unless by_path['/etc/tmpfiles.d/prefect-host-volume.conf']
abort('missing udev late-attach rule') unless by_path['/etc/udev/rules.d/99-prefect-host-volume.rules']
