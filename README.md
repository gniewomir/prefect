...
Tests: unified `./test.sh <suite>` (Acceptance, Lifecycle, Unit) — [docs/agents/testing.md](docs/agents/testing.md), [ADR-0036](docs/adr/0036-unified-test-entrypoint.md). Everything else lives under `internals/` (flat glanceable list): diagnostics, lint, ensure-components, Workload Setup, Purge, Stack, Components, and helpers. Same `--env` rule for Environment-scoped entrypoints. Layout and Host-local function names: [ADR-0032](docs/adr/0032-operator-surface-internals-and-host-function-names.md).
