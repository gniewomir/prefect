...
**Operator UX:** `park.sh` is everyday iteration (warn that Durables still bill). `teardown.sh` requires typing `teardown`. Retire silent full-wipe `destroy.sh` (point at Park vs Teardown).
