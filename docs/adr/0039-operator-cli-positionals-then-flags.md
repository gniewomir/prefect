...
**Shapes:** `./apply.sh --yes --env prod`; `./internals/ensure-workload.sh <name> [--env <slug>]`; `./test.sh <suite> [<case-selector>] [--verbose] [--env <slug>]`. Passthrough entrypoints with no named positionals (`./ssh.sh`) may declare `rest:` — known flags are peeled from anywhere; remaining tokens forward (so remote argv can start with `-`).
