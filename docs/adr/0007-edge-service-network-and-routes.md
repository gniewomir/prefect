...
Public Hosts get a mandatory **Edge** (Prefect-owned HTTP/HTTPS front door) and a Prefect-owned **Service Network**. Optional **Workloads** join that network and publish only through **Routes** (drop-in config the Edge includes). Unit names follow the role (`service-network`, `edge`, `edge-nginx`); tree is `internals/components/network/` + `internals/components/edge/`. TLS terminates at the Edge (ACME is Edge-owned and on-demand — ADR-0015); the first drop is HTTP :80 only, with Host bind mounts reserved for Routes and future certs. Empty Edge (no Routes) answers with a default 404/444 — not a holding page and not “don’t run.”
