...
- **Root operator entrypoints** (high-level operations on the Environment / platform as a whole): `apply.sh`, `park.sh`, `teardown.sh`, `ssh.sh`, and the unified test dispatcher `test.sh` (ADR-0036). Diagnostics, lint, ensure-components, Workload Setup, and Purge are not root entrypoints — they are internals (dev/debug/CI or cogs a future wider operation will compose). Scoped power (one Workload, one Component) belongs as options on platform-level entrypoints later, not as separate root scripts.
- **Root declarations and docs:** `environments/` (Environment intent — ADR-0033), `docs/`, and root `*.md` stay at root. Dotdirs stay at root.
- **`internals/`:** Stack (`terraform/`), operator helpers (`lib/`), Host Component ship surface (`components/`), test suites under `test/<suite>/` (ADR-0036), and a **flat** glanceable list of non-root operations (`diagnostics.sh`, `lint-*.sh`, `ensure-components.sh`, `workload-setup.sh`, `purge-workloads.sh`, …). Components must live under `internals/components/` so the Host copy tar never shares a tree with Stack/docs/tests/operator `lib/`.
