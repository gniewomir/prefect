...
Public Hosts pin Ubuntu 26.04 LTS. The container engine is Podman from Ubuntu’s packages, installed only through Initial Host Provisioning (engine package only — no `package_update`, no full `package_upgrade`, no Quadlet units). Apt indexes from the Host Image are enough to install; refresh on create may return once Prefect is out of dev-only iteration.
