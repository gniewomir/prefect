...
Public Hosts pin Ubuntu 26.04 LTS. The container engine is Podman from Ubuntu’s packages, installed only through Initial Host Provisioning (`package_update`, no full `package_upgrade`; engine package only — no Quadlet units).
