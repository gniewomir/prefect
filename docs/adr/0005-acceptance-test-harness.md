...
Operator checks of an applied Stack are **Acceptance Tests**: a root runner builds a shared fixture once, then executes numeric-prefixed capability-slice scripts as subprocesses (fail-fast; optional single-slice selector). They require a live Applied Stack (Host present) and are **non-destructive** to Stack lifecycle — they must not Park or Teardown. Stack lifecycle checks are **Lifecycle Tests** (`lifecycle-test.sh`), a separate opt-in suite (ADR-0016).
