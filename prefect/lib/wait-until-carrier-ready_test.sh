#!/usr/bin/env bash
# Unit tests: Carrier ready Host Volume mount wait (ADR-0031).
# Stubs cloud-init / sysctl / id / findmnt via PATH — no SSH.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
SCRIPT="${REPO_ROOT}/prefect/lib/wait-until-carrier-ready.sh"
...
STUBS="$(mktemp -d "${TMPDIR:-/tmp}/carrier-ready.XXXXXX")"
...
run_gate() {
  ...
  env PATH="${STUBS}:/usr/bin:/bin" \
    ...
    PREFECT_HOST_VOLUME_MOUNT_WAIT_SECONDS="${PREFECT_HOST_VOLUME_MOUNT_WAIT_SECONDS:-30}" \
    PREFECT_HOST_VOLUME_MOUNT_POLL_SECONDS="${PREFECT_HOST_VOLUME_MOUNT_POLL_SECONDS:-1}" \
...
export PREFECT_HOST_VOLUME_MOUNT_WAIT_SECONDS=10
export PREFECT_HOST_VOLUME_MOUNT_POLL_SECONDS=1
...
# --- timeout: message points at prefect-host-volume.service ---
export FINDMT_SUCCEED_AFTER=999999
export PREFECT_HOST_VOLUME_MOUNT_WAIT_SECONDS=2
export PREFECT_HOST_VOLUME_MOUNT_POLL_SECONDS=1
if run_gate; then
  fail "gate should fail when mount never appears"
fi
grep -q 'Host Volume mount /var/lib/prefect missing' "${STUBS}/err" \
  || fail "expected mount-missing message, got: $(cat "${STUBS}/err")"
grep -q 'prefect-host-volume.service' "${STUBS}/err" \
  || fail "expected pointer to prefect-host-volume.service, got: $(cat "${STUBS}/err")"
pass "on timeout points at prefect-host-volume.service"
