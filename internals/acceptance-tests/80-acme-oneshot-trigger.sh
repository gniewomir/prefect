...
before="$(host_ssh \
  "cat /var/lib/host-volume/components_data/edge/acme/last-run 2>/dev/null || echo none")"
sleep 2

"${REPO_ROOT}/internals/ensure-components.sh" --env "${PLATFORM_ENV:-test}"

want="$(host_ssh "cat /var/lib/host-volume/components_data/edge/acme/want-list")"
if [[ -n "${EXPECTED}" ]]; then
...
for _ in $(seq 1 30); do
  after="$(host_ssh \
    "cat /var/lib/host-volume/components_data/edge/acme/last-run 2>/dev/null || echo missing")"
...
while IFS= read -r fqdn; do
  ...
  for _ in $(seq 1 120); do
    if host_ssh \
      "test -f /var/lib/host-volume/components_data/edge/certs/${fqdn}/fullchain.pem \
       && test -f /var/lib/host-volume/components_data/edge/certs/${fqdn}/privkey.pem"; then
      pem_ok=1
      break
    fi
    sleep 2
