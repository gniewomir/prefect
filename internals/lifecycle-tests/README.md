...
./internals/lifecycle-tests.sh park-apply      # one slice (substring match on the filename)
./internals/lifecycle-tests.sh teardown        # Teardown wipe (prompts for exact 'teardown')
./internals/lifecycle-tests.sh --env test      # same Environment as omitting --env (`default` also aliases)
./internals/lifecycle-tests.sh --env prod      # Lifecycle against another Environment (explicit)
```
...
1. Add `lifecycle-tests/NN-short-name.sh`.
