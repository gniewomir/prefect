...
for n in trash-a reclaim-b keep-me purge-me; do
  rm -rf "/var/lib/host-volume/components_data/workloads/${n}"
  rm -f "/var/lib/host-volume/components_data/edge/routes/${n}.conf"
  rm -f /var/lib/host-volume/components_data/edge/routes/"${n}"--*
...
if [[ -z "${CERT_FQDN}" ]]; then
  CERT_FQDN="$(host_ssh \
    "ls -1 /var/lib/host-volume/components_data/edge/certs 2>/dev/null | head -1" || true)"
fi
...
if [[ -n "${ROUTE_FQDN}" ]]; then
  host_ssh bash -s <<REMOTE
set -euo pipefail
printf '%s\n' 'leftover-route' > /var/lib/host-volume/components_data/edge/routes/purge-me--${ROUTE_FQDN}.conf
chown platform:platform /var/lib/host-volume/components_data/edge/routes/purge-me--${ROUTE_FQDN}.conf
REMOTE
fi

"${REPO_ROOT}/internals/purge-workloads.sh" --env "${PLATFORM_ENV:-test}"

host_ssh "test ! -e /var/lib/host-volume/components_data/workloads/purge-me" \
...
host_ssh "test -f /var/lib/host-volume/components_data/workloads/keep-me/manifest.json" \
  || fail "Purge must not touch Intent stop keep-me"
host_ssh "test -f /var/lib/host-volume/components_data/workloads/reclaim-b/manifest.json" \
  || fail "Purge must not touch Intent run reclaim-b"

if [[ -n "${CERT_FQDN}" ]]; then
  host_ssh \
    "test -f /var/lib/host-volume/components_data/edge/certs/${CERT_FQDN}/fullchain.pem \
     && test -f /var/lib/host-volume/components_data/edge/certs/${CERT_FQDN}/privkey.pem" \
